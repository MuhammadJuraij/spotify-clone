import React from 'react'
import { assets } from '../assets/assets'
import { useNavigate } from 'react-router-dom'

function Navbar() {
    const navigate=useNavigate()
    return (
        <>
            <div className='w-full flex items-center justify-between font-semibold '>
                <div className='flex items-center gap-2'>
                    <img onClick={()=>navigate(-1)} className='w-8 bg-black p-2 rounded-full cursor-pointer  ' src={assets.arrow_left} alt="" />
                    <img onClick={()=>navigate(1)} className='w-8 bg-black p-2 rounded-full cursor-pointer  ' src={assets.arrow_right} alt="" />
                </div>
                <div className='flex items-center gap-2'>
                    <p className='bg-white text-black text-[15px] px-4 py-1 rounded-full hidden md:block cursor-pointer'>Explore Premium</p>
                    <p className='bg-black  px-3 py-1 text-[15px] rounded-full cursor-pointerpointer hidden md:block'>Install App</p>
                    <p className='bg-purple-500  text-black w-7 h-7 rounded-full flex items-center justify-center'>D</p>
                </div>
            </div>

            <div className='mt-4 flex gap-2 items-center'>
                <p className='bg-white text-black px-4 py-1 rounded-2xl text-[15px] cursor-pointer'>All</p>
                <p className='bg-black px-4 py-1 rounded-2xl text-[15px] cursor-pointer'>Music</p>
                <p className='bg-black px-4 py-1 rounded-2xl text-[15px] cursor-pointer'>Podcasts</p>
            </div>
        </>
    )
}

export default Navbar
